...
#' Now let's work on the [C] matrix.

#' Calculate the 1nd derivative operator. Here 3 nodes are commonly used. Calculate a centered difference, at the middle of the stencil.
order = 1  
t_stencil = (0.0:1.0:2.0) .* dt  
t0_stencil = 1.0 .* dt 
stencil_1 = calculate_weights(order, t0_stencil, t_stencil)
...
t_stencil = (0.0:1.0:2.0) .* dt  
t0_stencil = 0.0 .* dt 
stencil_1_forward = calculate_weights(order, t0_stencil, t_stencil)
stencil_1_backward = reverse(stencil_1_forward)
