...
#https://docs.google.com/document/d/1F4uLh0lVtDN1RgVFNob0hkWG_mhHZWYGn27HoeyIyHg/edit?usp=sharing
#Compare Julia numerical solution to Duhamel's integral calculation for
#arbitrary loadings.
...
#*************
#now calculate Duhamel's integral solution

ωD=ωn*sqrt(1-ζ^2)   #define natural frequency including damping

#define unit impulse response
function UnitImpulseResponse(th, m, ωD, ζ)
    h = 1 ./(m.*ωD) .*exp.(-ζ.*ωn.*th) .*sin.(ωD.*th)
end

uTheory = zeros(length(t_eq))  #initialize response array
...
for i = 1:length(t_eq)-1
    ...
    uTheory[i+1:end] = uTheory[i+1:end] .+ h  #sum impulse contribution

end


#compare Julia and convolution responses
plot(t,u, xlabel="time [sec.]", ylabel="displacement, u [m]", label="Julia")

plot!(t_eq,uTheory, label="Duhamel's Integral")
